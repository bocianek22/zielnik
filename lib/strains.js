...
export async function listStrains() {
  ...
  return sql()`
    SELECT s.id, s.producer, s.name, s.type, s.final_rating::float8 AS final_rating, s.taste, s.created_by,
      pool_key(s.id, s.producer, s.thc, s.cbd) AS pool_key, s.thc::float8 AS thc, s.cbd::float8 AS cbd, s.kind, s.terpenes, s.description,
...
export async function parseCommon(body) {
  ...
  return { fields: { name, producer, type, finalRating: fr, taste, kind, thc, cbd, terpenes, description } };
}
