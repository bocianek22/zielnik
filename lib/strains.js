...
export async function listStrains() {
  ...
  return sql()`
    SELECT s.id, s.producer, s.name, s.type, s.final_rating::float8 AS final_rating, s.taste, s.created_by,
      pool_key(s.id, s.producer, s.thc, s.cbd) AS pool_key, s.price_per_g::float8 AS price_per_g, s.batch, to_char(s.expires_on, 'YYYY-MM-DD') AS expires_on, s.thc::float8 AS thc, s.cbd::float8 AS cbd, s.kind, s.terpenes, s.description,
      ...
      COALESCE(json_agg(json_build_object(
        ...
        'current', us.current_amount, 'remaining', COALESCE(up.remaining_to_buy, 0), 'notes', us.notes, 'effects', us.effects
...
export async function listTests(strainId) {
  await ensureDb();
  return sql()`SELECT t.id, t.user_id AS "userId", u.username, t.note, (t.data IS NOT NULL) AS "hasPhoto",
               ...
               WHERE t.strain_id = ${strainId} ORDER BY t.created_at DESC, t.id DESC`;
}
