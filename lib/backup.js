...
export async function buildBackup() {
  ...
  return {
    ...
    prescriptions: await rows(q`SELECT COALESCE(jsonb_agg(to_jsonb(t)), '[]') AS rows FROM prescriptions t`),
    symptom_log: await rows(q`SELECT COALESCE(jsonb_agg(to_jsonb(t)), '[]') AS rows FROM symptom_log t`),
