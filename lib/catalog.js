...
export async function syncCatalog(rawRows, source) {
  ...
  for (const raw of rawRows.slice(0, 2000)) {
    ...
    await q`INSERT INTO market_catalog (producer, name, thc, cbd, kind, availability, source, run_id, last_seen, active)
            VALUES (${producer}, ${name}, ${thc}, ${cbd}, ${kind}, ${availability}, ${source}, ${runId}, now(), TRUE)
...
export async function listCatalog() {
  await ensureDb();
  return sql()`SELECT id, producer, name, thc::float8 AS thc, cbd::float8 AS cbd, kind, availability, source, active,
