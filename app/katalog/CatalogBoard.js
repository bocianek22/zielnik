...
export default function CatalogBoard({ items, owned, isAdmin }) {
  ...
  const shown = useMemo(() => {
    ...
    return items.filter((i) => (!onlyActive || i.active) && (!q || `${i.name} ${i.producer} ${i.kind || ''}`.toLowerCase().includes(q)));
  ...
  async function add(i) {
    ...
    try {
      await api('/api/strains', 'POST', {
        producer: i.producer, name: i.name, type: 'nieokreślony', kind: i.kind || '', thc: i.thc ?? '', cbd: i.cbd ?? '',
