...
export const POST = safe(async (req) => {
  ...
  const q = sql();
  const [row] = await q`INSERT INTO strains (producer, name, type, final_rating, taste, thc, cbd, kind, terpenes, description, price_per_g, batch, expires_on, created_by)
                        VALUES (${f.producer}, ${f.name}, ${f.type}, ${f.finalRating}, ${f.taste}, ${f.thc}, ${f.cbd}, ${f.kind},
                                ${JSON.stringify(f.terpenes)}::jsonb, ${f.description}, ${f.price}, ${f.batch}, ${f.expires}::date, ${user.id})
                        RETURNING id`;
